...
class Solution:
    def merge(self, nums1: List[int], m: int, nums2: List[int], n: int) -> None:
        ...
        numbers_1 = list(reversed(nums1[:m])) # O(n)
        numbers_2 = list(reversed(nums2)) # O(m)

        i = 0 # our 'output' index.
        while len(numbers_1) and len(numbers_2): # O(n+m)
        ...
        remnants = reversed([*numbers_1, *numbers_2]) # O(n+m)
